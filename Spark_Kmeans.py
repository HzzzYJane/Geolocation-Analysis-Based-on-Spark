...
def closestPoint(the_centroids, point, dis):
    ...
    for index in range(len(the_centroids)):
        temp_dis = EuclideanDistance(the_centroids[index], point) if dis == "E" else GreatCircleDistance(the_centroids[index], point)
...
if __name__ == "__main__":
    ...
    # check if command line has enough argument
    if len(sys.argv) < 5:
    ...
    centroids_initial = all_data.takeSample(False, input_k, randint(1, 100))
    ...
    # save text file
    all_data.map(lambda p:  (closestPoint(centroids, p, input_dis), p)) \
	    .map(lambda x: str(x[0]) + " " + str(x[1][0]) + " " + str(x[1][1]) ) \
	    .saveAsTextFile(output + "/" + str(input_k) + input_dis + "/" + "points")
    sc.parallelize(centroids) \
      .map(lambda x: str(x[0]) + " " + str(x[1])) \
      .saveAsTextFile(output + "/" + str(input_k) + input_dis + "/" + "clusters")
